...
# 1. إعدادات الصفحة الفائقة وإخفاء كل زوائد ستريمليت
st.set_page_config(
...
# 2. نظام إدارة اللغات (Translations Dictionary)
...
translations = {
    'English': {
        ...
        'group_1': 'Metabolic Stats',
        'group_2': 'Physical Stats',
        'btn': 'INITIALIZE AI SCAN',
        'conf': 'AI Confidence',
        'res_pos': 'HIGH RISK | DIABETIC',
        'res_neg': 'LOW RISK | STABLE',
    ...
    'Arabic': {
        ...
        'res_pos': 'خطر مرتفع | High Risk',
        'res_neg': 'خطر منخفض | آمن',
...
# 3. حقن الألوان والتصميم (The Visual Replica CSS)
st.markdown(f"""
    <style>
    ...
    /* كروت المدخلات (Cyan Glow Box) */
    [data-testid="stForm"] {{
        background-color: #0c141d !important;
        border: 1.5px solid #00f2ea44 !important;
...
# تحميل الموديل والـ scaler من ملف model.py
...
with col_input:
    # استخدام Form لضمان عدم التحديث التلقائي المزعج
    with st.form("mustafa_replica_form"):
...
# 7. قسم النتائج (The Neon Result Card)
if submit:
    ...
    st.markdown(f"""
        <div style="border: 2px solid {res_color}; background: {res_color}11; padding: 40px; border-radius: 25px; text-align: center; box-shadow: 0 0 20px {res_color}33;">
            <h1 style="color: {res_color}; background:none; -webkit-text-fill-color:{res_color}; margin:0; font-size: 2.5rem;">
                {res_text}
            </h1>
            <p style="font-size: 1.5rem; margin-top: 15px;">{T['conf']}: <b>{score:.1f}%</b></p>
